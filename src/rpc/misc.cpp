...
{
    FunctionProfile profileThis("getinfo", - 1, 10);
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total fabcoin balance of the wallet\n"
    ...
    obj.push_back(Pair("testnet_no_dns",       Params().NetworkIDString() == CBaseChainParams::TESTNET_NODNS));
#ifdef ENABLE_WALLET
    if (pwallet) {
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwallet && pwallet->GetCScript(scriptID, subscript)) {
            ...
            for (const CTxDestination& addr : addresses)
                a.push_back(CFabcoinAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The fabcoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The fabcoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Fabcoin address and we have full public key:
        CFabcoinAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are fabcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) fabcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CFabcoinAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The fabcoin address to use for the signature.\n"
...
{
    FunctionProfile profileThis("getmemoryinfo", - 1, 10);
    /* Please, avoid using the word "pool" here in the RPC interface or help,
     * as users will undoubtedly confuse it with the other "memory pool"
     */
    if (request.fHelp || request.params.size() > 1)
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "fabcoin-cli and the GUI. There is no server-side difference."
        );
